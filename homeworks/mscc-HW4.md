...
2. Branch vs trunk-based approaches
    ...
    - _Disadvantages:_ To make possible a trunk-based approach, it's necessary to have regulations that ensures code review and "owner files" to make sure that uncorrect commits will not be accepted. Even with the use of a software tool to calculate and advice if a change is wrong or worths the effort along with the code review made by developers, an uncorrect commit can be submitted, since there's no official way to prevent this.
